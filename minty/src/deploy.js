...
async function deployContract(name, symbol) {
  ...
  const SpamReports = await hardhat.ethers.getContractFactory("SpamReports");
  const spamReports = await SpamReports.deploy();

  await minty.deployed();
  await auction.deployed();
  await spamReports.deployed();

  await writeDeploymentInfo(spamReports, "spamReports.json");
